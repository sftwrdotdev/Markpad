...
- **Windows Authenticode signing without SignPath** — one-time setup step 8 is the only path wired here. With it unconfigured, neither the portable `.exe` nor the `*-setup.exe` NSIS installer is signed, and users may see a SmartScreen warning. Minisign verification by the updater is independent of it.
