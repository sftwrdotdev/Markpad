...
Obsidian 的嵌入语法对本地文件同样有效——`![[lightmode.png]]`——解析方式一致。把图片拖进编辑器会自动写好引用；把 `.md` 文件拖到任一侧都会在标签页里打开它。在访达或资源管理器里复制一个图片*文件*，直接粘贴也算——它会存到文档旁边并写好引用，而不是粘成一段路径。
...
行内也可以：$\ce{H2O}$，单位写成 $\pu{123 kJ//mol}$。

由 KaTeX 渲染，mhchem 注册在同一个实例上，所以 `\ce` 和 `\pu` 在任何写公式的地方都能用。
...
- **点标题旁边的箭头**折叠它，下面的内容一起收起。折叠状态按文档记住。编辑器侧也能从自己的行号槽折叠同样的标题，和它本来就有的列表折叠并存。
